...
margins_records |>
  inner_join(teams_info, by = "team") |>
  ggplot(aes(avg_margin, pct)) +
  geom_point(aes(col = team), size = 4, shape = "square", show.legend = F) +
  ggrepel::geom_text_repel(aes(label = abb), size = 3) +
  scale_color_manual(values = team_hex) +
  geom_hline(yintercept = mean(margins_records$pct), linetype = "dashed", alpha = 0.25) +
  geom_vline(xintercept = mean(margins_records$avg_margin), linetype = "dashed", alpha = 0.25) +
  scale_x_continuous(breaks = seq(0, 10, by = 0.25)) +
  scale_y_continuous(breaks = seq(0, 100, by = 5)) +
  labs(x = "Average Run Margin", y = "Win Percentage",
       title = "Which teams are winning their close games?")
```
...
# new work
cubs_pk = end_games |>
  filter(win_team == "Chicago Cubs") |>
  slice_max(date, n = 1, with_ties = F) |>
  pull(game_pk)

hit_criteria = c("Double", "Home Run", "Single", "Triple")

ab_criteria = c("Double", "Field Error", "Fielders Choice", "Fielders Choice Out",
                "Flyout", "Forceout", "Grounded Into DP", "Groundout", "Home Run",
                "Pop Out", "Sac Fly", "Single", "Strikeout", "Triple")
```
