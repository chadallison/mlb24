...
get_seven_group_pct = function(team, group) {
  data = end_games |>
    filter(home_team == team | away_team == team) |>
    mutate(game_num = as.integer(rank(date)),
           seven_group = cut(game_num, breaks = seq(0, 168, by = 7), labels = F)) |>
...
team_sevens |>
  ...
  geom_col(aes(fill = pct), show.legend = F, width = 0.75) +
  facet_wrap(vars(team)) +
  scale_fill_gradient(low = "#CF9D9D", high = "#7A9C74") +
  labs(x = "Group of Seven", y = NULL,
       title = "Team Records in Groups of Seven") +
